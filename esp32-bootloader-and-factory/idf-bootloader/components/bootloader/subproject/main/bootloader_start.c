...
#endif // CONFIG_BOOTLOADER_SKIP_VALIDATE_IN_DEEP_SLEEP
...
static int selected_boot_partition(const bootloader_state_t *bs) {
  ...
  if (bootloader_common_get_reset_reason(0) != DEEPSLEEP_RESET) {
    ...
    if (bootloader_common_check_long_hold_gpio(CONFIG_BOOTLOADER_NUM_PIN_FACTORY_RESET, CONFIG_BOOTLOADER_HOLD_TIME_GPIO) == 1) {
      ESP_LOGI(TAG, "Detected condition for factory reset.");
      bool ota_data_erase = false;
      // Assuming we have a Factory app.
      boot_index = FACTORY_INDEX;

#ifdef CONFIG_BOOTLOADER_OTA_DATA_ERASE
      ota_data_erase = true;
#endif // CONFIG_BOOTLOADER_OTA_DATA_ERASE
